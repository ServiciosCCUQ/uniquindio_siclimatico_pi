...
def saveImage(settings, width, height, quality, diskSpaceToReserve):
    ...
    subprocess.call('raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -a 1036 -ae +25+25 -o %s' % (settings, width, height, quality, filename), shell=True)
...
def motion():
    ...
    print('[UQ] - Iniciando Captura de Fotos Nocturnas ...')
    ...
    while (True):
        ...
        f_valida = datetime.now()
        
        if f_valida.hour >= 6 and f_valida.hour <= 17:
            continue 
